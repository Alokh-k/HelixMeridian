...
export default function PersonalRound() {
  ...
  useEffect(() => {
    // Setup browser speech recognition & synthesis
    ...
    // Start interview by triggering initial prompt
    handleUserResponse("Hello, I am ready to start the interview.");
  }, [candidateId]);
  ...
  const speakText = (text: string) => {
    ...
    utterance.onend = () => {
      setSpeaking(false);
      // Start listening once the interviewer finishes speaking
      startListening();
    };
    ...
    try {
      ...
      if (data.response) {
        if (data.response.includes('[ROUND_COMPLETED]')) {
          speakText("Thank you. Personal round completed. Transitioning to technical round.");
          setTimeout(() => {
            router.push(`/interview/${candidateId}/technical`);
          }, 3000);
        } else {
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100 flex flex-col justify-between p-8 font-sans">
      <header className="border-b border-zinc-900 pb-4 max-w-4xl w-full mx-auto flex justify-between items-center">
        <div>
          ...
          <p className="text-xs text-zinc-400">Round 1: Personal Fit</p>
        </div>
      ...
      <main className="max-w-xl w-full mx-auto flex flex-col items-center justify-center space-y-8 flex-1">
        ...
        <div className="flex items-center justify-center space-x-2 h-32">
          {[...Array(5)].map((_, idx) => (
            <span
              key={idx}
              className={`w-2 rounded-full bg-indigo-500 transition-all duration-300 ${
                speaking
                  ? 'h-16 animate-pulse'
                  : isListening
                  ? 'h-8 animate-bounce'
        ...
        <div className="text-center space-y-2">
          <h2 className="text-lg font-semibold text-zinc-200">
            {speaking ? 'Interviewer Speaking...' : isListening ? 'Listening to you...' : loading ? 'Thinking...' : 'Ready'}
          </h2>
          <p className="text-xs text-zinc-500">
            Speak naturally. The interviewer will respond and prompt you when finished.
          </p>
        </div>
